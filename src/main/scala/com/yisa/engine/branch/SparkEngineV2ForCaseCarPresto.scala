...
class SparkEngineV2ForCaseCarPresto(line: String, tableName: String, zkHostport: String, PrestoHostPort: String) extends Runnable {
  ...
  override def run() {
    ...
    var connP = DriverManager.getConnection(PrestoURL, PrestoUser, null)

    var stmtP = connP.createStatement()
    ...
    var sqlI = "insert into scajcb_result (s_id,j_id,t_id) values(?,?,?)";
    var pstmt = conn.prepareStatement(sqlI);

    var sql2 = "insert into scajcb_progress (jobid,total) values(?,?)";
    ...
    try {
      ...
      if (map.length > 0) {
        ...
        sql.append("select arbitrary(solrid) as solrid,sum(type) as types from (")
      ...
      case ex: Exception => {
        println("涉车案件串并-presto异常 ： " + ex.printStackTrace())
      }
  ...
  def gsonArrayFunc(inputBean: InputBean, tableName: String, map5: HashMap[Int, Int]): String = {
    ...
    sb.append(" SELECT   arbitrary(solrid) as solrid, platenumber , ")
