...
class SparkEngineV2ForCaseCar {

  def searchCaseCar(sparkData: Dataset[Row], sqlContext: SparkSession, line: String, tableName: String, zkHostport: String): Unit = {
    ...
    if (map.length > 0) {
      ...
      if (df != null) {
        ...
        var sqlUnion = "SELECT first(solrid) as solrid, platenumber, concat_ws(',',collect_set(type)) as types from df group by platenumber"
