...
import com.yisa.engine.modules.SparkEngineModulesForVer5
...
object SparkEngineVer5 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var tableName = "global_temp.pass_info" + getDateMid()
    var tableNameAll = "global_temp.pass_info_all"

    var tableNameWithoutGlobal = "pass_info" + getDateMid()
    var tableNameAllWithoutGlobal = "pass_info_all"
    ...
    inittable2.createGlobalTempView(tableNameWithoutGlobal)
    ...
    loadAllData.createGlobalTempView(tableNameAllWithoutGlobal)
    ...
    val actorSystem = ActorSystem("sparkAkkaSystem")
    ...
    while (true) {
      ...
      while (rei.hasNext()) {
        ...
        val SparkEngineModules = SparkEngineModulesForVer5
        ...
        try {
          taskType match {
            ...
            case CommonTaskType.FrequentlyCar => {
              ...
              SparkEngineModules.FrequentlyCar(line, sparkSession, tableName, threadPool, prestoTableName, frequentlyCarResultTableName, zkHostPort, cacheDays, prestoHostPort)

            }
            //以图搜车
            case CommonTaskType.SearchCarByPic => {

              SparkEngineModules.SearchCarByPic(actorSystem, line, sparkSession, tableName, zkHostPort, cacheDays, tableNameAll, threadPool, prestoHostPort, prestoTableName)

            }
            //相似车牌
            case CommonTaskType.SearchSimilarPlate => {

              SparkEngineModules.SearchSimilarPlate(inittable2, line, sparkSession, threadPool, tableName, prestoTableName, zkHostPort, cacheDays, tableNameAll, prestoHostPort)

            }
            //多点碰撞
            case CommonTaskType.MultiPoint => {
              SparkEngineModules.MultiPoint(inittable2, line, sparkSession, threadPool, tableName, prestoTableName, zkHostPort, cacheDays, prestoHostPort)

            }
            //涉车案件串并
            case CommonTaskType.CaseCar => {

              SparkEngineModules.CaseCar(inittable2, line, sparkSession, threadPool, tableName, prestoTableName, zkHostPort, cacheDays, prestoHostPort)

            }
            //终结伪基站
            case CommonTaskType.EndStation => {

              SparkEngineModules.EndStation(inittable2, line, sparkSession, threadPool, tableName, prestoTableName, zkHostPort, cacheDays, prestoHostPort)

            }
            //同行车辆
            case CommonTaskType.TogetherCar => {

              SparkEngineModules.TogetherCar(sparkSession, line, tableName, zkHostPort)

            }
            //导出卡口
            case CommonTaskType.ExportLocation => {

              SparkEngineModules.ExportLocation(inittable2, sparkSession, line, tableName, zkHostPort)
