...
//    loadAllData.createOrReplaceTempView(tableNameAll)
    ...
    //    RedisConnectionPool.returnResource(RedisConnectionPool.getJedis(zkHostPort))
        ...
        try {
          taskType match {
            ...
            case CommonTaskType.FrequentlyCar => {
              ...
              SparkEngineModulesForVer21.FrequentlyCar(line, sparkSession, tableName, threadPool, prestoTableName, frequentlyCarResultTableName, zkHostPort, cacheDays_2, prestoHostPort)

            }
            //以图搜车
            case CommonTaskType.SearchCarByPic => {
              SparkEngineModulesForVer21.SearchCarByPic(line, sparkSession, tableName, zkHostPort, cacheDays4SearchCar, tableNameAll, threadPool, prestoHostPort, prestoTableName, allYearidByBrandid, allYearidByModelid, AllYearids)
            ...
            case CommonTaskType.SearchSimilarPlate => {
              ...
              SparkEngineModulesForVer21.SearchSimilarPlate(inittable2, line, sparkSession, threadPool, tableName, prestoTableName, zkHostPort, cacheDays_2, tableNameAll, prestoHostPort)
            }
            //多点碰撞
            case CommonTaskType.MultiPoint => {
              ...
              SparkEngineModulesForVer21.MultiPoint(inittable2, line, sparkSession, threadPool, tableName, prestoTableName, zkHostPort, cacheDays_2, prestoHostPort)
            ...
            case CommonTaskType.CaseCar => {
              ...
              SparkEngineModulesForVer21.CaseCar(inittable2, line, sparkSession, threadPool, tableName, prestoTableName, zkHostPort, cacheDays_2, prestoHostPort)
            ...
            case CommonTaskType.EndStation => {
              ...
              SparkEngineModulesForVer21.EndStation(inittable2, line, sparkSession, threadPool, tableName, prestoTableName, zkHostPort, cacheDays_2, prestoHostPort)

            }
            //同行车辆
            case CommonTaskType.TogetherCar => {

              SparkEngineModulesForVer21.TogetherCar(sparkSession, line, tableName, zkHostPort)

            }
            //导出卡口
            case CommonTaskType.ExportLocation => {

              SparkEngineModulesForVer21.ExportLocation(inittable2, sparkSession, line, tableName, zkHostPort)

            }
            case CommonTaskType.Nocturnal => {

              SparkEngineModulesForVer21.ConcealOneselfByDayAndMarchByNight(line, sparkSession, tableName, zkHostPort, cacheDays, tableNameAll, threadPool, prestoHostPort, prestoTableName)
            }
            case CommonTaskType.PassTimeout => {
              SparkEngineModulesForVer21.PassTimeout(sparkSession, line, tableName, zkHostPort)
            }
  ...
  def loadAlltimeData(sparkSession: SparkSession, hdfsPath: String, dataPath: String, cacheDays: Int): Dataset[Row] = {
