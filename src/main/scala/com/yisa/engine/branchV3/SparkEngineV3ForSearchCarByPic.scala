 ...
 * 以图搜车
 */
class SparkEngineV3ForSearchCarByPic(sqlContext: SparkSession, line: String, tableName: String, zkHostport: String) extends Runnable {
  ...
  override def run() {
    ...
    val resultData = executeSparkSQLNoPartition(sqlContext, sql1, zkHostport, job_id)
    val insertMySQLEnd = new Date().getTime
    //    val tempCount = resultData.count()
    val tempCount = resultData
