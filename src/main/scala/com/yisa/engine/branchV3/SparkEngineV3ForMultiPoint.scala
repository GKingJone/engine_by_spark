...
class SparkEngineV3ForMultiPoint (sparkData: Dataset[Row], sqlContext: SparkSession, line: String, tableName: String, zkHostport: String) extends Runnable {
 ...
 override def run() {
    ...
    if (map.length > 0) {
      ...
      if (df != null) {
        ...
        var sqlUnion = "SELECT first(solrid) as solrid, platenumber, concat_ws(',',collect_set(type)) as types from df group by platenumber order by types desc limit 1000"
        ...
        if (inputBeanRepair != null) {
          ...
          var sqlNot = "SELECT solrid, types FROM dfIs WHERE types NOT LIKE '%" + i + "%'"
  ...
  def gsonArrayFunc(inputBean: InputBean, typeC: String, tableName: String): String = {
    ...
    //    if (startTime != 0) {
    //      sb.append(" WHERE capturetime >= ").append(startTime)
    //    }
    //
    //    if (endTime != 0) {
    ...
    if (plateNumber != null && plateNumber != "") {
      sb.append(" AND plateNumber = ").append(plateNumber)
    }
