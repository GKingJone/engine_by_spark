...
class SparkEngineV3ForCaseCar(sparkData: Dataset[Row], sqlContext: SparkSession, line: String, tableName: String, zkHostport: String) extends Runnable  {
  
 override def run() {

    var conn = MySQLConnectManager.getConnet(zkHostport)
    conn.setAutoCommit(false)
    var sql = "insert into scajcb_result (s_id,j_id,t_id) values(?,?,?)";
