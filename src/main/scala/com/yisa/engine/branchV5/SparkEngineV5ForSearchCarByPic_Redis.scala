 ...
 * 以图搜车
 */
class SparkEngineV5ForSearchCarByPic_Redis extends Serializable {

  def SearchCarByPic(sqlContext: SparkSession, line: String, tableName: String, zkHostport: String, allYearidByBrandid: Map[Int, Set[Long]], allYearidByModelid: Map[Int, Set[Long]], AllYearids: scala.collection.mutable.ArrayBuffer[String]): Unit = {
    ...
    var m = yearids.foreach { yearid =>
      {
        for (dateid <- startTimeDateid to endTimeDateid) {
          ...
          for (index <- 0 to values.size - 1) {
            ...
            var timeCatch = true
            try {
              var capturetime = value.split("_")(3).toLong
